...
def run_ingestion_pipeline(doc_id: str, file_path: str, filename: str):
    ...
    try:
        ...
        for page_idx in range(1, total_pages + 1):
            ...
            # 1. Extract Facts using Gemini & FastEmbed
            page_facts = process_pdf_page(file_path, page_idx)
            
            for f in page_facts:
                # 2. Insert Fact into Database
                cur.execute(
                ...
                # 3. Vector Search: Find related facts from OTHER documents (Cosine Distance < 0.28)
                cur.execute(
                ...
                # 4. Adjudicate & Link (The Knowledge Graph Edges)
                for cand in candidates:
                    ...
                    if adj["relation_type"] != "insufficient_evidence":
                        # Sort IDs to prevent duplicate A->B and B->A edges
...
async def get_all_facts():
    # An endpoint for our React UI to grab everything
    conn = get_db()
    cur = conn.cursor()
    cur.execute("""
